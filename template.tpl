___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "type": "TAG",
  "id": "cvt_temp_public_id",
  "version": 1,
  "securityGroups": [],
  "displayName": "ReklamStore Dsp Conversion Pixel",
  "categories": ["AFFILIATE_MARKETING", "ADVERTISING", "CONVERSIONS", "LEAD_GENERATION", "MARKETING", "REMARKETING"],
  "brand": {
    "id": "brand_dummy",
    "displayName": "",
    "thumbnail": "data:image/jpeg;base64,"
  },
  "description": "Use the ReklamStore DSP Conversion Pixel template to easily track your campaigns\u0027 performance.",
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "type": "TEXT",
    "name": "price",
    "displayName": "Conversion Price Variable",
    "simpleValueType": true,
    "valueHint": "Price Variable Name ($)",
    "help": "Please Select Your Conversion Price Variable ($) by pressing the button on the right. If you haven\u0027t created conversion pixel value variable, create variable first.",
    "valueValidators": [
      {
        "type": "NON_EMPTY"
      }
    ]
  },
  {
    "type": "TEXT",
    "name": "clickId",
    "displayName": "Click ID Variable Name",
    "simpleValueType": true,
    "help": "Please Select Your Click Id Variable by pressing the button on the right. If you haven\u0027t created conversion pixel Click ID variable, create variable first.",
    "valueHint": "Click Id Variable Name",
    "valueValidators": [
      {
        "type": "NON_EMPTY"
      }
    ]
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

const log = require('logToConsole');
const getQueryParameters = require('getQueryParameters');
const sendPixel = require('sendPixel');
const encodeUriComponent = require('encodeUriComponent');
const query = require('queryPermission');

const onSuccess = () => {
  log('Conductrics: Request loaded successfully.');
  data.gtmOnSuccess();
};

const onFailure = () => {
  log('Conductrics: Request load failed.');
  data.gtmOnFailure();
};

const clickId = data.clickId;
const price = data.price;
const url = ("https://rtb.reklamdsp.com/log?action=conversion&key="+data.clickId+"&price="+data.price);

log(clickId,price);

if (encodeUriComponent('sendPixel', url)) {
  	log('sendPixel: ', url);
	sendPixel(url, onSuccess, onFailure);
  	onSuccess();
}

return data;


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "logging",
        "versionId": "1"
      },
      "param": [
        {
          "key": "environments",
          "value": {
            "type": 1,
            "string": "debug"
          }
        }
      ]
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "send_pixel",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urls",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "https://rtb.reklamdsp.com/*"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "get_url",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urlParts",
          "value": {
            "type": 1,
            "string": "any"
          }
        },
        {
          "key": "queriesAllowed",
          "value": {
            "type": 1,
            "string": "any"
          }
        }
      ]
    },
    "isRequired": true
  }
]


___TESTS___

scenarios: []


___NOTES___

Created on 27.02.2020 10:39:27


